...
# --- CSS APPLE ULTIMATE FINAL (STABLE) ---
st.markdown("""
    <style>
    ...
    /* BOTÓN LIMPIAR: DERECHA Y PEQUEÑO */
    ...
    /* BOTONES DE TAREA: 16:9 HORIZONTAL */
...
def update_console(msg):
    ...
    # Nota: No usamos rerun aquí para no detener la ejecución
...
try:
    ...
    # CONSOLA (CONTENEDOR VACÍO PARA ACTUALIZACIÓN EN VIVO)
    ...
    if execute_masivo:
        st.session_state.logs.append("› Initiating Global Pipeline...")
        render_console()
        conn = snowflake.connector.connect(**SF_PARAMS); cs = conn.cursor()
        for t in TAREAS:
            st.session_state.logs.append(f"› Processing: {t['tab']}...")
            render_console()
            run_task(t, drive_service, gc, cs, console_placeholder)
        cs.close(); conn.close()
        st.session_state.logs.append("› All tasks finished.")
        st.rerun()
            ...
            for i, t in enumerate(lista):
                with cols[i % 6]:
                    if st.button(t['tab'].replace('_', ' '), key=f"btn_{t['tab']}_{sid}"):
                        st.session_state.logs.append(f"› Syncing {t['tab']}...")
                        render_console()
                        conn = snowflake.connector.connect(**SF_PARAMS); cs = conn.cursor()
                        run_task(t, drive_service, gc, cs, console_placeholder)
                        cs.close(); conn.close()
                        st.session_state.logs.append(f"› {t['tab']} Done.")
                        st.rerun()
